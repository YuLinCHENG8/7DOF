...
int select_optimal_ik_golden(const double T_target[4][4],
                             ...
                             double q_best[N_JOINTS],
                             double psi_cur,
                             const pair<double,double>& intervals) {
    const double tol = 0.05 * (M_PI / 180.0);            // ~0.005°

    double best_score = INFINITY;
    int found = 0;

    //为解决计算时会跳变到臂角的边界上，需要限定一个区间来选择解
    double psi_district_tolerance = 15 * M_PI /180; //变化区间为15°
    // double lo = std::max(psi_cur - psi_district_tolerance, intervals.first);
    // double hi = std::min(psi_cur + psi_district_tolerance, intervals.second);
    //不做区间限定版本
    double lo = intervals.first;
    double hi = intervals.second;
    
    printf("[黄金分割] [%.2f°, %.2f°] 搜索中...\n",
...
int select_optimal_ik_golden_all_district(const double T_target[4][4],
    ...
    for(size_t k = 0;k<intervals.size();++k){
        ...
        printf("[黄金分割] 第%zu组 最优臂角: %.4f°, 得分: %.3f\n",
...
int main() {
    ...
    auto run_ik_cycle = [&]() {
        ...
        // select_optimal_ik_golden(T_fk, q_input, q_prev, q_ik, cur_arm_angle, current_psi_interval);
        printf("----------当前臂角区间为：%f, %f \n",
            current_psi_interval.first * 180/M_PI,current_psi_interval.second * 180/M_PI);
